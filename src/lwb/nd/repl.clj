...
(defn man
  ...
  (let [info (str/join \newline
                       ["lwb - Natural Deduction"
                        ...
                        "  e.g. (step-b :not-i 5)"
                        "- (swap symbol replacement)"
                        "  swaps symbol and replacement"
                        "  e.g. (swap '?1 'Q)"
                        "- (subclaim formula)"
                        "  inserts a subclaim as an intermediate proof obligation"
                        "  e.g. (subclaim '(at [j] (always (or A B))))"
                        "- (load-theorem id)"
                        "  loads the current proof as theorem with the id :id into the theorem storage"
                        "  e.g. (load-theorem :intermediate-step)"
                        "- (undo)"
